...
export function ProductCard({ product }: { product: Product & { id?: string, _id?: string } }) {
  ...
  return (
    <Link href={`/product/${product.id}`} className="block">
      {/* Perfect square card */}
      <div className="relative aspect-square w-full rounded-2xl overflow-hidden shadow-sm border border-gray-100 bg-white group cursor-pointer transition-all hover:-translate-y-0.5 hover:shadow-md">

        {/* Image — fills ~65% of height */}
        <div className="absolute inset-0 bottom-[35%]">
          {product.image ? (
            ...
            <div className="h-full w-full bg-muted flex items-center justify-center text-muted-foreground/20 text-4xl font-bold uppercase tracking-widest">
        ...
        <div className="absolute bottom-0 left-0 right-0 h-[35%] bg-white px-2.5 pt-2 pb-2 flex flex-col justify-between">
          ...
          <div className="flex items-center justify-between">
            ...
            {product.isAvailable === false ? (
              <span className="text-[9px] text-gray-400 font-semibold">Unavailable</span>
            ) : (
              <button
                onClick={handleAddToCart}
                className="flex items-center gap-0.5 bg-primary text-white text-[10px] sm:text-xs font-bold px-2.5 py-1 rounded-full hover:bg-primary/90 transition-colors z-10"
              >
